...
v0.0.6a - Tower Detection Update
---------------------------------
...
class minecartDriver
{
  ...
  // 
    ...
    camera.takeSnapshot(camera__TOWERYELLOW);
    if(camera.largestObject.exists && camera.largestObject.centerY < 250){
      int codeCenterX = camera.largestObject.centerX;
      int codeCenterY = camera.largestObject.centerY;

      int midPoint = 151;
      int maxDeviance = 20;

      if(codeCenterX > midPoint-maxDeviance && codeCenterX < midPoint+maxDeviance){
        rightMotor.setVelocity(cruiseVel, percent);
        leftMotor.setVelocity(cruiseVel, percent);
      } else {
      
        if(codeCenterX < midPoint){
          rightMotor.setVelocity(cruiseVel+15, percent);
          leftMotor.setVelocity(cruiseVel-5, percent);
        }

        if(codeCenterX > midPoint){
          leftMotor.setVelocity(cruiseVel+15, percent);
          rightMotor.setVelocity(cruiseVel-5, percent);
        }
